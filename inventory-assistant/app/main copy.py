...
def fast_inventory_answer(question: str) -> ChatResponse | None:
    ...
    if wants_dci_connectivity:
        site_aliases = {
            "chicago": "dc-chicago", "london": "dc-london", "singapore": "dc-singapore",
            "mumbai": "dc-mumbai",
        }
        site_key = next((key for name, key in site_aliases.items() if name in text), None)
        result = inventory_query("dci_connectivity", site_key=site_key)
        if not result["rows"]:
            return ChatResponse(
                answer="No DCI connectivity records matched that data-center scope.",
                tools_used=["inventory_query:dci_connectivity:fast"], model="database-fast-path"
            )
        grouped: dict[str, list[dict[str, Any]]] = {}
        for row in result["rows"]:
            pair = f"{row['source_site']} ↔ {row['target_site']}"
            grouped.setdefault(pair, []).append(row)
        lines = []
        for pair, paths in grouped.items():
            detail = "; ".join(
                f"{p['provider']} {p['bandwidth_gbps']} Gbps: {p['source_core']} {p['source_interface']} → "
                f"{p['target_core']} {p['target_interface']} · {p['status']} · "
                f"{p['utilization_pct']}% utilized · {p['latency_ms']} ms"
                for p in paths
            )
            lines.append(f"• {pair}\n  {detail}")
        return ChatResponse(
            answer=(f"DCI topology ({len(grouped)} data-center pairs, {len(result['rows'])} diverse circuits):\n"
                    + "\n".join(lines) + "\nAll records are simulated for this demo."),
            tools_used=["inventory_query:dci_connectivity:fast"], model="database-fast-path"
        )
    ...
    if wants_server_connectivity:
        ...
        for row in result["rows"]:
            attachment_lines.append(
                f"• {row['redundancy_role'].title()} path: {row['server_interface']} "
                f"(MAC {row['mac_address']}, IP {row['server_ip_addresses']}) → "
                f"{row['switch_name']} port {row['switch_interface']} · "
                f"VLAN {row['vlan_id']} ({row['vlan_name']}) · {row['connection_status']}"
            )
        row = result["rows"][0]
        answer = (
...
def inventory_query(
    operation: Literal[
        ...
        "server_connectivity", "connectivity_paths", "dci_connectivity"
    ],
    ...
    with database() as connection:
        if operation == "summary":
            ...
            """, (site_key, site_key, server_key, server_key, server_key))
        elif operation == "dci_connectivity":
            result = rows(connection, """
                SELECT link_key,source_site_key,source_site,source_core,source_interface,
                       target_site_key,target_site,target_core,target_interface,provider,circuit_id,
                       round(bandwidth_bps/1000000000.0)::bigint bandwidth_gbps,
                       utilization_pct,latency_ms,packet_loss_pct,status,redundancy_group,
                       last_seen_at,is_simulated
                FROM inventory.data_center_backbone_overview
                WHERE (%s::text IS NULL OR source_site_key=%s OR target_site_key=%s)
                ORDER BY source_site,target_site,provider
...
SEMANTIC_ENTITIES: dict[str, dict[str, Any]] = {
    ...
    "dci_connectivity": {
        "source": "inventory.data_center_backbone_overview",
        "fields": ["link_key","source_site_key","source_site","source_core","source_interface",
                   "target_site_key","target_site","target_core","target_interface","provider","circuit_id",
